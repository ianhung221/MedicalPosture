...
export function createPosePipeline(config = POSTURE_CONFIG) {
  ...
  let lastKnownY = 0.5; let missingSince = null;
  ...
  return {
    startCalibration() { stabilizer.reset(POSTURE_STATES.CALIBRATING); return calibration.start(); },
    process(landmarks, timestamp) {
      ...
      if (before.active) { const after = calibration.add(features.currentRatio, { valid: isValidCalibrationFrame(features, config) }); const rawState = after.completed ? classifyPosture(features, after.thresholdRatio, config) : POSTURE_STATES.CALIBRATING; if (after.completed) stabilizer.reset(rawState); const stableState = after.completed ? stabilizer.push(rawState) : POSTURE_STATES.CALIBRATING; return { rawState, stableState, calibration: after, features }; }
      const rawState = classifyPosture(features, before.thresholdRatio, config); const stableState = stabilizer.push(rawState); return { rawState, stableState, calibration: before, features };
    },
    reset() { smoother.reset(); calibration.reset(); stabilizer.reset(); lastKnownY = 0.5; missingSince = null; },
    getSnapshot() { return { calibration: calibration.getSnapshot(), stabilizer: stabilizer.getSnapshot(), lastKnownY, missingSince }; },
  };
}
