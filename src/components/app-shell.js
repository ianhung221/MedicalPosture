...
export function renderAppShell(pageRenderer, activeRoute) {
  ...
  app.innerHTML = `${header()}<main id="main-content" class="main-content" tabindex="-1"></main>${bottomNav(activeRoute)}<div class="toast" role="status" aria-live="polite">示範模式：目前顯示概念操作流程</div>`;
  pageRenderer(document.querySelector('#main-content'));
