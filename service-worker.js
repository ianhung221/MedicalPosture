const CACHE_NAME = 'posture-health-shell-v37';
...
const APP_SHELL = [
  ...
  './src/utils/assessment-navigation.js',
  './src/posture/posture-episode-tracker.js', './src/posture/reminder-policy.js', './src/posture/reminder-presentation.js', './src/posture/walking-safety-policy.js', './src/posture/safety-walking-stabilizer.js', './src/imu/imu-posture-classifier.js',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys
    .filter((key) => (key.startsWith(CACHE_PREFIX) && key !== CACHE_NAME)
      || (key.startsWith(AI_CACHE_PREFIX) && key !== AI_CACHE_NAME)
      || (key.startsWith(THREE_D_CACHE_PREFIX) && key !== THREE_D_CACHE_NAME))
...
self.addEventListener('fetch', (event) => {
  ...
  const isThreeDAsset = url.origin === self.location.origin && (url.pathname.includes('/assets/models/imu-neutral-head.glb') || url.pathname.includes('/assets/vendor/three-r185/'));
  if (isThreeDAsset) {
    event.respondWith(caches.open(THREE_D_CACHE_NAME).then(async (cache) => {
  ...
  const isLargeAiAsset = url.origin === self.location.origin && (url.pathname.includes('/assets/models/pose_landmarker_') || url.pathname.includes('/assets/vendor/mediapipe/wasm/'));
  if (isLargeAiAsset) {
